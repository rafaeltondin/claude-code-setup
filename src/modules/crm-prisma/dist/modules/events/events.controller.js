...
function emitSSE(event, data) {
    ...
    for (const client of sseClients) {
        try {
            ...
            sseClients.delete(client);
            removed++;
            logger_1.logger.warn('[SSE] Cliente removido por erro de escrita', {
                event: safeEvent,
                errorMessage: err.message,
            });
        }
    }
    logger_1.logger.debug('[SSE] Evento emitido', {
        event,
...
router.get('/stream', (req, res) => {
    ...
    // Registrar cliente
    sseClients.add(res);
    logger_1.logger.info('[SSE] Novo cliente conectado', {
        ip: req.ip,
    ...
    req.on('close', () => {
        ...
        logger_1.logger.info('[SSE] Cliente desconectado', {
            ip: req.ip,
            totalClientes: sseClients.size,
        });
    });
